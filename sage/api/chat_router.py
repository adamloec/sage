...
async def create_chat_session(request: Request, create_request: ChatSessionCreate):
    ...
    if request.app.state.model_manager.get_current_model() is None:
        request.app.state.model_manager.load_model(create_request.model_config)
    
    session = request.app.state.session_manager.create_session()
    return ChatSessionResponse(
        session_id=session.session_id,
        created_at=session.created_at,
        model_config=request.app.state.model_manager.get_current_config(),
...
async def send_message(request: Request, session_id: str, msg_request: ChatMessageRequest):
    """Send a message in a chat session"""
    session = request.app.state.session_manager.get_session(session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    
    model = await request.app.state.model_manager.get_current_model()
...
async def update_model(request: Request, model_config: ModelConfig):
    ...
    try:
        request.app.state.model_manager.load_model(model_config)
        return model_config
...
async def handle_window_close(request: Request):
    ...
    request.app.state.model_manager.unload_current_model()
